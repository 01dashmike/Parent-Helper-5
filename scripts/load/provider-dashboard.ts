/**
 * Load test script for /api/provider/metrics endpoint (provider dashboard)
 * 
 * Usage: npx tsx scripts/load/provider-dashboard.ts
 * 
 * Note: This endpoint may require authentication. Adjust based on your auth setup.
 */

const BASE_URL = process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000";
const TOTAL_REQUESTS = parseInt(process.env.LOAD_TEST_REQUESTS || "150", 10);
...
async function makeRequest(providerId: number = 1): Promise<{ status: number; latency: number; error?: string }> {
  const startTime = Date.now();
  
  try {
    const response = await fetch(`${BASE_URL}/api/provider/metrics?provider_id=${providerId}`, {
      method: "GET",
      headers: {
...
async function runLoadTest(): Promise<void> {
  console.log(`\n🚀 Starting load test for /api/provider/metrics`);
  console.log(`   Base URL: ${BASE_URL}`);
  console.log(`   Total requests: ${TOTAL_REQUESTS}`);
  console.log(`   Concurrent requests: ${CONCURRENT_REQUESTS}`);
  console.log(`   ⚠️  Note: This endpoint may require authentication\n`);
  ...
  for (let batch = 0; batch < Math.ceil(TOTAL_REQUESTS / CONCURRENT_REQUESTS); batch++) {
    ...
    for (let i = 0; i < batchSize; i++) {
      // Use provider ID 1 for testing (adjust if needed)
      const providerId = 1;
      batchPromises.push(
        makeRequest(providerId).then((response) => {
  ...
  console.log(`\n\n📊 Results for /api/provider/metrics`);
