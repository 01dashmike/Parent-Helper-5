...
import type { 
  ...
  NutritionEquipment, 
  BlogPostSummary,
  NUTRITION_STAGE_META 
} from "@/lib/wellness/types";
import NutritionRecipeFinder from "./NutritionRecipeFinder";

const STAGE_META: typeof NUTRITION_STAGE_META = {
  pregnancy: {
    label: "Pregnancy",
    icon: "🤰",
...
export default function NutritionPageClient({
  ...
  const handleStageChange = useCallback((stage: NutritionStage) => {
    ...
    router.push(`/health-wellness/nutrition?${params.toString()}`, { scroll: false });
  }, [router, searchParams]);
