...
export default function GrowthHubPanel({
  ...
  useEffect(() => {
    fetch(`/api/provider/features/list-active-features?providerId=${providerId}`)
      .then((res) => res.json())
      .then((data) => {
        if (data.success) {
          setEntitlements({
            featuredListing: data.entitlements.featuredListing,
            verifiedBadge: data.entitlements.verifiedBadge,
            premiumAnalytics: data.entitlements.premiumAnalytics,
          });
        }
      });
  }, [providerId]);
